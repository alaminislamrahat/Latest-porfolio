...
const Navbar = () => {
  ...
  return (
    ...
    className="fixed z-50 flex flex-col justify-between w-full h-full px-10 uppercase bg-black text-white/80 py-28 gap-y-10 md:w-1/2 md:left-1/2">
        <div className="flex flex-col text-5xl gap-y-2 md:text-6xl lg:text-7xl 2xl:text-8xl">
            {["home","services", "about", "work", "contact"].map((section,index)=>(
                <div key={index} ref={(el) => (linkRef.current[index] = el)}>
                    <a className="transition-all duration-300 cursor-pointer hover:text-white" href="">{section}</a>
                </div>
            ))}
        </div>
        ...
        className="flex flex-col flex-wrap justify-between gap-8 md:flex-row">
            <div className="font-light">
                <p className="text-white/50 tracking-wider">Email</p>
                <p className="text-xl tracking-widest lowercase text-pretty">alaminislamrahat@gmail.com</p>
            </div>
            <div className="font-light">
